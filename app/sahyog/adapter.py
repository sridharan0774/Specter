...
class LocalPackageTransport(SahyogTransport):
    ...
    def _build_evidence_chain(
        ...
        fallback_url = ChainRegistry.get_adapter(summary.chain if 'summary' in locals() and summary and hasattr(summary, 'chain') else "TRON").get_address_explorer_url(endpoint_address) if endpoint_address else "#"
        ...
        chain_steps = [
            ...
            },
            {
                ...
                "value": explorer_urls[0] if explorer_urls else fallback_url,
...
class SahyogAdapter:
    ...
    def build_sahyog_package(
        ...
        package = {
            ...
            "legal_compliance": {
                "section_94_bnss_requisition": {
                    "statute": "Section 94 of Bharatiya Nagarik Suraksha Sanhita, 2023 (BNSS) / former Sec 91 CrPC",
                    "authority": "Ministry of Home Affairs (MHA) / Indian Cybercrime Coordination Centre (I4C) Protocol",
                    "purpose": f"Legal Requisition demanding account holder KYC, IP log history, deposit receipts, and withdrawal settlement particulars for endpoint wallet '{endpoint_address}' at {vasp_name}.",
                    "status": "READY_FOR_INVESTIGATOR_SIGNATURE",
                },
                "section_65b_bsa_certificate": {
                    "statute": "Section 65B of Bharatiya Sakshya Adhiniyam, 2023 (BSA) / former Sec 65B Indian Evidence Act",
                    "certificate_id": f"BSA-65B-{uuid.uuid4().hex[:8].upper()}",
                    "evidence_digest_sha256": f"sha256-{uuid.uuid4().hex}",
                    "audit_timestamp": now_str,
                    "assertion": "Certifies that the electronic records, on-chain trace logs, and evidence ledger contained in this package were generated by SPECTER in the ordinary course of forensic analysis and remain untampered.",
