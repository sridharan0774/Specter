    ...
    for a in alerts_db:
        ...
        result.append(
            VelocityAlert(
                ...
                minimum_delta_t=float(m.get("minimum_delta_t")) if m.get("minimum_delta_t") is not None else None,
                average_delta_t=float(m.get("average_delta_t")) if m.get("average_delta_t") is not None else None,
                maximum_delta_t=float(m.get("maximum_delta_t")) if m.get("maximum_delta_t") is not None else None,
                initial_transfer_amount=float(m.get("initial_transfer_amount", 0.0)),
                downstream_activity_amount=float(m.get("downstream_activity_amount", 0.0)),
