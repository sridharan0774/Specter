...
class VelocityAlert(BaseModel):
    ...
    minimum_delta_t: Optional[float] = Field(None, description="Shortest delta_t between successive transfers in seconds (None if insufficient sequential transfers)")
    average_delta_t: Optional[float] = Field(None, description="Average delta_t between transfers in seconds")
    maximum_delta_t: Optional[float] = Field(None, description="Longest delta_t between transfers in seconds")
    initial_transfer_amount: float = Field(0.0, description="Initial observed transfer amount from starting wallet")
    downstream_activity_amount: float = Field(0.0, description="Downstream observed activity amount across subsequent hops")
