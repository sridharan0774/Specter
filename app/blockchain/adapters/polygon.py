...
logger = logging.getLogger("specter.polygon")
...
class PolygonAdapter(BlockchainAdapter):
    """
    Adapter for Polygon PoS blockchain ingestion via Polygonscan REST provider API.
    Supports native POL/MATIC transactions and ERC-20 token transfers (USDT, USDC).
    """
    ...
    def __init__(self, api_key: Optional[str] = None, timeout: float = 12.0):
        self.api_key = api_key or settings.POLYGONSCAN_API_KEY
    ...
    async def get_transactions(
        ...
        try:
            ...
            logger.warning(f"Error calling Polygonscan API: {e}")
    ...
    async def get_token_transfers(
        self,
        address: str,
        token_contract: Optional[str] = POLYGON_USDT_CONTRACT,
        limit: int = 50,
        min_timestamp: Optional[int] = None,
        max_timestamp: Optional[int] = None,
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"Error calling Polygonscan token transfers API: {e}")
    ...
    async def get_transaction(self, tx_hash: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(f"Error calling Polygonscan eth_getTransactionByHash: {e}")
        return None
    ...
    def normalize_transaction(self, raw_tx: Dict[str, Any]) -> NormalizedTransactionBase:
        ...
        asset = raw_tx.get("tokenSymbol", "POL").upper()
        decimals = int(raw_tx.get("tokenDecimal", 18 if asset in ["POL", "MATIC"] else 6))
