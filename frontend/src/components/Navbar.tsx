      ...
      <div className="max-w-[1440px] mx-auto px-6 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center space-x-6">
          ...
          <div className="flex items-center bg-slate-100 p-0.5 rounded border border-slate-200 text-xs">
            <button
              ...
              className={`px-3 py-1 rounded font-medium transition-colors duration-150 flex items-center space-x-1.5 ${
                isLiveMode
                  ? 'bg-white text-slate-900 shadow-xs font-semibold'
                  : 'text-slate-500 hover:text-slate-800'
              }`}
            >
              <span className={`w-1.5 h-1.5 rounded-full ${isLiveMode ? 'bg-emerald-600' : 'bg-slate-400'}`} />
              <span>Live Mode</span>
            </button>
            <button
            ...
            >
              <span className={`w-1.5 h-1.5 rounded-full ${!isLiveMode ? 'bg-amber-600' : 'bg-slate-400'}`} />
              <span>Demo Mode</span>
            </button>
          </div>
        </div>
        ...
        <nav className="flex items-center space-x-1">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-[#3730A3]' : 'text-slate-400'}`} />
                <span>{tab.label}</span>
              </button>
            );
          })}
        ...
        <div className="flex items-center space-x-3 text-xs">
          ...
          <div className="flex items-center space-x-1.5 text-slate-600 px-2 py-1">
            <span className={`w-2 h-2 rounded-full ${healthStatus === 'online' ? 'bg-emerald-600' : 'bg-amber-500'}`} />
